 ...
 */
export function Refrescar({ titulo = "Sincronizar ahora" }: { titulo?: string }) {
  return (
    <form action={sincronizarAhora}>
      <button
        title={titulo}
        aria-label={titulo}
        className="flex size-11 items-center justify-center rounded-full border-[1.5px] border-borde bg-white text-tinta-2 transition hover:bg-beige hover:text-pasto sm:size-10"
      >
